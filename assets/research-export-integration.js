(function() {
  ...
  function references() {
    return (currentPlan?.customer_research?.sources || []).filter(source => /^https?:\/\//i.test(source?.url || ''));
  }
  ...
  function exportPlanText() {
    ...
    if (requirement.business_challenges) lines.push(`业务问题：${requirement.business_challenges}`);
    if (requirement.learner_context) lines.push(`学员基础：${requirement.learner_context}`);
    if (requirement.preferences) lines.push(`实施要求：${requirement.preferences}`);
    ...
    if (sources.length) lines.push('', '附：公开资料参考', ...sources.map(source => `[${source.source_id}] ${source.title || '公开资料'}${source.published_at ? `（${source.published_at}）` : ''}｜${source.url}`));
    return lines.join('\n');
  }
  ...
  async function exportResearchDocx() {
    ...
    const info = [
      ...
      ['业务问题', requirement.business_challenges || ''],
      ['学员基础', requirement.learner_context || ''],
      ['实施要求', requirement.preferences || ''],
      ['安排说明', requirement.assumptions?.join('；') || '']
    ...
    const sources = references();
    let sourceRelationships = '';
    if (sources.length) {
      body += wP('附：公开资料参考', 'Heading1');
      sources.forEach((source, i) => {
        const id = `source${i + 1}`;
        const label = `[${source.source_id}] ${source.title || '公开资料'}${source.published_at ? `（${source.published_at}）` : ''}`;
        body += `<w:p><w:hyperlink r:id="${id}"><w:r><w:rPr><w:color w:val="0563C1"/><w:u w:val="single"/></w:rPr><w:t xml:space="preserve">${xmlEsc(label)}</w:t></w:r></w:hyperlink></w:p>`;
        sourceRelationships += `<Relationship Id="${id}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="${xmlEsc(source.url)}" TargetMode="External"/>`;
      });
    }
    ...
    zip.folder('word').folder('_rels').file('document.xml.rels', `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/>${sourceRelationships}</Relationships>`);
