...
import {issueSession} from '../edge-functions/lib/admin-auth.js';

test('authenticated import, duplicate protection, publication and withdrawal use persisted batches',async()=>{
 const originalFetch=globalThis.fetch,imports=[],users=[],guard=[];
 const env={ADMIN_BOOTSTRAP_USERNAME:'owner',ADMIN_BOOTSTRAP_PASSWORD:'test-password-long-enough',ADMIN_SESSION_SECRET:'x'.repeat(48),CLOUDBASE_ENV_ID:'test',CLOUDBASE_API_KEY:'test'};
 globalThis.fetch=async(input,options={})=>{
  const url=new URL(input),table=url.pathname.split('/').pop(),method=options.method||'GET';let result=[];
  if(table==='wb_identity_links')result=[];
  else if(table==='wb_login_guard'){
   if(method==='POST'){guard.push(JSON.parse(options.body));result=null;}
   else if(method==='PATCH'){Object.assign(guard[0],JSON.parse(options.body));result=null;}
   else result=guard;
  }
  else if(table==='wb_admin_users'){
   if(method==='POST'){const user=JSON.parse(options.body);users.push(user);result=null;}
   else if(method==='PATCH'){Object.assign(users.find(u=>u.id===url.searchParams.get('id').slice(3)),JSON.parse(options.body));result=null;}
   else result=users.filter(u=>!url.searchParams.get('username')||u.username===url.searchParams.get('username').slice(3));
  }
 ...
 try{
  const login=await post({action:'login',username:'owner',password:env.ADMIN_BOOTSTRAP_PASSWORD});assert.equal(login.status,200);const cookie=login.headers.get('Set-Cookie').split(';')[0];
  ...
  const userCookie=`wb_session=${await issueSession(env,'user','visitor')}`;
