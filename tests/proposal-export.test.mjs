...
test('actual page, copy and generated Word share paragraph analysis and reference links', async () => {
  const nodes = new Map();
  ...
  for (const file of ['research-session.js', 'training-workflow.js', 'proposal-display.js', 'research-display.js']) vm.runInContext(await readFile(new URL(`../assets/${file}`, import.meta.url), 'utf8'), context);
  ...
  context.fixture = { requirement_summary: { audience: '中层干部', theme: '数字化经营', days: '1天', sessions: '1讲', goals: '提升数据决策能力' }, proposal: {
    project_background: { text: '客户公开资料提及数据应用。[S1]本项目据此连接业务背景与培训目标。', source_ids: ['S1'] },
    client_needs_analysis: { text: '根据本次培训需求，建议围绕数据判断与业务应用形成学习目标。', source_ids: [] },
    design_logic: { text: '先建立分析框架，再通过“数据决策”模块练习岗位应用，配课方案如下。', source_ids: [] }
  }, formal_schedule: [{ day: '第1天', period: '上午', module: '数据决策', course_title: '数据驱动经营', teacher_name: '库内老师', reason: '回应数据应用问题，并形成经营判断能力。' }], customer_research: { sources: [{ source_id: 'S1', title: '年度报告', url: 'https://example.test/report?a=1&b=2', evidence: '不应堆在正文中的原始长摘要。' }] } };
  ...
  for (const section of Object.values(context.fixture.proposal)) assert.ok(xml.includes(section.text));
  assert.doesNotMatch(xml, /原始长摘要/);
  assert.match(xml, /<w:hyperlink r:id="source1">/);
  assert.match(rels, /Target="https:\/\/example.test\/report\?a=1&amp;b=2" TargetMode="External"/);
